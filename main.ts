// namespace label defaults to English and will be localized to Hiragana via _locales
//% color=#FF8C00 icon="\uf031" block="Hiragana"
namespace hiraloc {
    /**
     * Run the handler when a chat command is typed.
     * @param cmd chat command
     */
    //% block="on chat %cmd do %handler"
    //% cmd.shadow="text" cmd.defl="start"
    //% blockAllowReadOnly=1
    ...
    export function agentForward(blocks: number): void {
        agent.move(FORWARD, blocks);
    }

    //% block="agent move back %blocks blocks"
    //% blocks.min=1 blocks.max=100 blocks.defl=1
    export function agentBack(blocks: number): void {
        agent.move(BACK, blocks);
    }

    //% block="agent move right %blocks blocks"
    //% blocks.min=1 blocks.max=100 blocks.defl=1
    export function agentRight(blocks: number): void {
        agent.move(RIGHT, blocks);
    }

    //% block="agent move left %blocks blocks"
    //% blocks.min=1 blocks.max=100 blocks.defl=1
    export function agentLeft(blocks: number): void {
        agent.move(LEFT, blocks);
    }

    //% block="agent move up %blocks blocks"
    //% blocks.min=1 blocks.max=100 blocks.defl=1
    export function agentUp(blocks: number): void {
        agent.move(UP, blocks);
    }

    //% block="agent move down %blocks blocks"
    //% blocks.min=1 blocks.max=100 blocks.defl=1
    export function agentDown(blocks: number): void {
        agent.move(DOWN, blocks);
    }

    //======== Agent place ========
    //% block="agent place block in front"
    export function agentPlaceFront(): void {
        agent.place(FORWARD);
    }

    //% block="agent place block below"
    export function agentPlaceDown(): void {
        agent.place(DOWN);
    }
}
